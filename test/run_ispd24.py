...
def get_data(file):
  ...
  with open(file, 'r') as file_open:
    ...
    for line in file_lines:
      ...
      if items[0] == "Sum" and items[4] == "um":
        total_disp = float(items[3])
      elif items[0] == "Delta":
        delta_hpwl = float(items[3])
  ...
  return [total_disp, max_disp, delta_hpwl, total_runtime]
...
def run_and_eval(design_name):
  ...
  template_name = "__skyline_ispd_template.tcl"
  ...
  total_disp, max_disp, delta_hpwl ,runtime = get_data(log_name)

  print(f"{design_name}")
  print(f"TotalDisp : {total_disp} um")
  print(f"Max  Disp : {max_disp} um")
  print(f"DeltaHPWL : {delta_hpwl:.2f} %")
  print(f"Runtime   : {runtime} s")

  sp.call(f"rm {log_name}", shell=True)
  sp.call(f"rm {tcl_name}", shell=True)

  return [design_name, total_disp, max_disp, delta_hpwl, runtime]

csv_name = "ispd24_results.csv"
if os.path.exists(csv_name):
  sp.call(f"rm {csv_name}", shell=True)

with open(csv_name, "a") as csv_file:
  csv_wr = csv.writer(csv_file)
  csv_wr.writerow(["Benchmark", "TotalDisp (um)", "Maxdisp (um)", "Delta HPWL", "Runtime"])
