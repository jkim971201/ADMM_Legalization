...
dbLefReader::init()
{
  ...
  lefrSetViaRuleCbk(this->lefViaRuleCbk);

  lefrSetPropCbk(this->lefPropCbk);
}
...
int
dbLefReader::lefViaRuleCbk(lefrCallbackType_e c, lefiViaRule* viaRule, lefiUserData ud)
{
  dbTech* tech = (dbTech*) ud;
  tech->createNewViaRule(viaRule);
  return 0;
}

int
dbLefReader::lefPropCbk(lefrCallbackType_e c, lefiProp* prop, lefiUserData ud)
{
  dbTech* tech = (dbTech*) ud;
  if(prop->hasString())
  {
    if(std::string(prop->propName()) == "LEF58_CELLEDGESPACINGTABLE")
    {
      std::string text = prop->string();
      // Regular expression matching three values after "EDGETYPE".
      std::regex reg("EDGETYPE (\\d+) (\\d+) ([0-9.]+)"); 
      std::sregex_iterator it(text.begin(), text.end(), reg); 
      std::sregex_iterator end;   
  
      if(it == end)
        unsupportSyntax(prop->propName());
      else
      {
        while(it != end)
        {
          ...
          int type2 = std::stoi(match.str(2));
          double spacing = std::stod(match.str(3));
          tech->addEdgeSpacing(type1, type2, spacing);
          ++it;
        }
      }
    }
