    ...
    bookShelfFlag_ (false)
{
  ...
  bsReader_      = std::make_shared<dbBookShelfReader>(types_, tech_, design_);
}
...
{
  const int bookshelf_dbu = bsReader_->dbuBookShelf();
  design_->writeBookShelf(filename, bookshelf_dbu);
}
...
int 
dbDatabase::getBookshelfDbu() const
{
  return bsReader_->dbuBookShelf();
}
