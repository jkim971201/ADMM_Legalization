...
dbDefReader::dbDefReader(
  std::shared_ptr<dbTypes>  types, 
...
dbDefReader::init()
{
  ...
  // Region
  defrSetRegionCbk(this->defRegionCbk);

  // Group 
  defrSetGroupCbk(this->defGroupCbk);
  defrSetGroupNameCbk(this->defGroupNameCbk);
  defrSetGroupMemberCbk(this->defGroupMemberCbk);
...
dbDefReader::defSNetCbk(defrCallbackType_e c, defiNet* net, defiUserData ud)
{
  ...
  std::string nameStr = std::string(net->name());
  dbNet* dbnet = design->getNetByName(nameStr);

  if(dbnet == nullptr)
    dbnet = design->getNewSNet(nameStr);
...
dbDefReader::defComponentStartCbk(defrCallbackType_e c, int number, defiUserData ud)
{
  ...
  dbDesign* design = (dbDesign*) ud;
  design->componentStartCbk(number);
  return 0;
}
...
int 
dbDefReader::defRegionCbk(defrCallbackType_e c, defiRegion* rg, defiUserData ud)
{
  checkType(c);
  dbDesign* design = (dbDesign*) ud;
  design->addNewRegion(rg);
  return 0;
}

int 
dbDefReader::defGroupNameCbk(defrCallbackType_e c, const char* name, defiUserData ud)
{
  checkType(c);
  dbDesign* design = (dbDesign*) ud;
  design->createNewGroup(name);
  return 0;
}

int 
dbDefReader::defGroupCbk(defrCallbackType_e c, defiGroup* group, defiUserData ud)
{
  checkType(c);
  dbDesign* design = (dbDesign*) ud;
  design->fillGroup(group);
  return 0;
}

int 
dbDefReader::defGroupMemberCbk(defrCallbackType_e c, const char* member, defiUserData ud)
{
  checkType(c);
  dbDesign* design = (dbDesign*) ud;
	design->addGroupMember(member);
  return 0;
}
