...
Graph::Graph(std::vector<GPCell*>& cellList)
{
  ...
  {
    GPCell* cell1 = cellList[i];
    for(int j = i + 1; j < nVertex; j++)
    {
      if(i != j)
      {
        GPCell* cell2 = cellList[j];
  ...
  for(auto& e : edges_)
  {
    ...
    e.v2()->addEdge(&e);
//    // Debugging
//    GPCell* cell1 = cellList[e.v1()->id()];
//    GPCell* cell2 = cellList[e.v2()->id()];
  }
...
FMPartitioner::FMPartitioner(std::vector<GPCell*>& cellList, double balance, int& curNumClutser)
{
