 ...
 * @return hash
 */
__device__ int getHashSparse(int val, int index, int prime) 
{
  int h1 = val % prime;
 ...
 */
__global__ void computeEdgesSumSparse(DeviceStructure deviceStructures) 
{
 ...
 */
__global__ void computeCommunityWeightSparse(DeviceStructure deviceStructures) 
{
 ...
 */
__device__ int prepareHashArraysSparse(const int    community, 
                                 const int    prime, 
                                 const float  weight, 
                                       float* hashWeight, 
                                       int*   hashCommunity,
                                 const int    hashTablesOffset) 
{
  ...
  do {
    curPos = hashTablesOffset + getHashSparse(community, it++, prime);
                                   ...
                                   const float  vertexToCommunity) 
{
  ...
  float gain = vertexToCommunity / MSparse
             + vertexEdgesSum[vertex] * (currentCommunitySum - communitySum) / (2 * MSparse * MSparse);

	//printf("communitySum : %f currentCommunitySum : %f vertexToCommunity: %f vertexEdgesSum[vertex]: %f M : %f gain : %f\n", 
	//		    communitySum, currentCommunitySum, vertexToCommunity, vertexEdgesSum[vertex], M, gain);
...
__device__ void computeMoveSparse(const int    V,
		                              const float  MSparse,
		                              const int    minSize,
                                  const int*   vertices, 
                                  const int    prime, 
                                  ...
                                  int*   bestCommunities) 
{
  ...
  if(vertexIndex < V) 
  {
    ...
    while(neighbourIndex < upperBound) // This is ok because the edge IDs are in the sorted order
    {
      ...
      if(neighbour != vertex) 
      {
        curPos = prepareHashArraysSparse(community, prime, weight, hashWeight, hashCommunity, hashTablesOffset);
        if(community == currentCommunity)
          atomicAdd(&vertexToCurrentCommunity[threadIdx.y], weight);
      }

			if( communitySize[currentCommunity] < minSize)
				printf("Community i : %d Community j : %d\n", currentCommunity, community);
      if( (community != currentCommunity) && (communitySize[currentCommunity] < minSize) )
      {
        // computeGain does not change anything
        // Gain is the cost of moving community i to community j
        float gain = computeGainSparse(vertex, 
						                           MSparse,
        ...
        if(gain > bestGain || (gain == bestGain && community < bestCommunity) ) 
        {
          ...
          bestCommunity = community;
					printf("bestCommunity : %d bestCommunitySize : %d\n", bestCommunity, communitySize[bestCommunity]);
        }
      }
  ...
  //if(threadIdx.x == 0 && bestGain - vertexToCurrentCommunity[threadIdx.y] / M > 0) 
    if(threadIdx.x == 0 && communitySize[currentCommunity] < minSize)
		{
      newVertexCommunity[vertex] = bestCommunity;
			printf("CommuniySize : %d currentCommunity: %d bestCommunitySize: %d bestCommunity: %d\n", 
					    communitySize[currentCommunity], currentCommunity, communitySize[bestCommunity], bestCommunity);
		}
    else 
...
__global__ void computeMoveSharedSparse(const int   V, 
		                                    const float MSparse,
		                                    const int   minSize,
                                        const int*  vertices, 
                                        const int   prime, 
...
__global__ void computeMoveGlobalSparse(const int    V, 
		                                    const float  MSparse,
		                                    const int    minSize,
                                        const int*   vertices, 
                                        const int    prime, 
 ...
 * @param deviceStructures structures kept in device memory
 */
__global__ void updateVertexCommunitySparse(int V, 
                                            const int* vertices, 
                                            DeviceStructure deviceStructures) 
{
...
__global__ void updateOriginalToCommunitySparse(DeviceStructure deviceStructures) 
{
  int vertex = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
...
struct isInBucketSparse
{
  isInBucketSparse(int llowerBound, int uupperBound, int* eedgesIndex) 
  {
...
{
	float MSparse = hostStructures.M;
	printf("Start Removal M: %f\n", MSparse);

  int V = hostStructures.V;
  // blocksNumber(int V, int numThreadPerVertex)
  // ~= (V * numThreadPerVertex) / 128
  computeEdgesSumSparse<<<blocksNumber(V, WARP_SIZE), dim3{WARP_SIZE, THREADS_PER_BLOCK / WARP_SIZE}>>>(deviceStructures);
  ...
  auto   predicate            = isInBucketSparse(buckets[lastBucketNum], buckets[lastBucketNum + 1], hostStructures.edgesIndex);
  ...
  while(totalGain >= minGain) 
  {
    ...
    for(int bucketNum = 0; bucketNum < bucketsSize - 2; bucketNum++)
    {
      ...
      auto predicate       = isInBucketSparse(buckets[bucketNum    ], 
                                              buckets[bucketNum + 1], 
      ...
      if(verticesInBucket > 0) 
      {
        ...
        computeMoveSharedSparse<<<blocksNum, blockDimension, sharedMemSize>>>(verticesInBucket, 
						                                                                  MSparse,
						                                                                  minSize,
                                                                              partition, 
                                                                              prime,
        ...
        updateVertexCommunitySparse<<<blocksNumber(V, 1), THREADS_PER_BLOCK>>>(verticesInBucket, 
                                                                               partition,
                                                                               deviceStructures);
        ...
        computeCommunityWeightSparse<<<blocksNumber(V, 1), THREADS_PER_BLOCK>>>(deviceStructures);
      }
    }
    ...
    if(verticesInBucket > 0) 
    {
      ...
      computeMoveGlobalSparse<<<blocksNum, lastBlockDimension, sharedMemSize>>>(verticesInBucket, 
					                                                                      MSparse,
					                                                                      minSize,
                                                                                partition, 
                                                                                lastBucketPrime,
  ...
  updateOriginalToCommunitySparse<<<blocksNumber(hostStructures.originalV, 1), THREADS_PER_BLOCK>>>(deviceStructures);
  ...
  DeviceStructure& deviceStructures)
{
  ...
  computeEdgesSumSparse<<<blocksNumber(V, WARP_SIZE), 
...
//  // Revert for following removal process
//  thrust::fill(thrust::device, 
//               deviceStructures.communitySize, 
//               deviceStructures.communitySize + V, 1.0);

  return numSparseCluster;
}
