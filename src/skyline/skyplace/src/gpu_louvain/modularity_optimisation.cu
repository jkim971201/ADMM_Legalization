 ...
 */
__global__ void computeEdgesSum(DeviceStructure deviceStructures) 
{
 ...
 */
__global__ void computeCommunityWeight(DeviceStructure deviceStructures) 
{
...
__global__ void updateOriginalToCommunity(DeviceStructure deviceStructures) 
{
  int vertex = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
...
__global__ void calculateToOwnCommunity(DeviceStructure deviceStructures) 
{
  int verticesPerBlock = blockDim.y;
  int concurrentNeighbours = blockDim.x;
  float edgesSum = 0;
  ...
  HostStructure&   hostStructures) 
{
  ...
  HANDLE_ERROR( cudaMemcpy(&(hostStructures.num_edge_final), deviceStructures.E, 
                 sizeof(int), cudaMemcpyDeviceToHost) );

  HANDLE_ERROR( cudaMemcpy(&(hostStructures.num_vertex_final), deviceStructures.V, 
                 sizeof(int), cudaMemcpyDeviceToHost) );


  int num_vertex_final    = hostStructures.num_vertex_final;
  int num_edge_final      = hostStructures.num_edge_final;
  auto& edge_final        = hostStructures.edge_final;
  auto& edge_index_final  = hostStructures.edge_index_final;
  auto& edge_weight_final = hostStructures.edge_weight_final;

  edge_final.resize(num_edge_final);
  edge_index_final.resize(num_vertex_final);
  edge_weight_final.resize(num_edge_final);

  HANDLE_ERROR( cudaMemcpy(edge_final.data(), deviceStructures.edges,
                 sizeof(int) * num_edge_final, cudaMemcpyDeviceToHost) );

  HANDLE_ERROR( cudaMemcpy(edge_index_final.data(), deviceStructures.edgesIndex,
                 sizeof(int) * num_vertex_final, cudaMemcpyDeviceToHost) );

  HANDLE_ERROR( cudaMemcpy(edge_weight_final.data(), deviceStructures.weights,
                 sizeof(float) * num_edge_final, cudaMemcpyDeviceToHost) );
}
