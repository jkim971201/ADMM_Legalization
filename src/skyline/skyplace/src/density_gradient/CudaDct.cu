...
#include "CudaDctUtil.h"
#include "CudaDct.h"

#define TPB 16

__global__ void dct2d_preprocess(const cufftReal *x, cufftReal *y, const int M, const int N, const int halfN)
{
...
__global__ __launch_bounds__(TPB *TPB, 10) void dct2d_postprocess(const cufftComplex* V, 
                                                                        cufftReal* y, 
                                                                  const int M, 
                                                                  const int N,
                                                                  const int halfM, 
...
__global__ __launch_bounds__(TPB *TPB, 8) void idct2d_preprocess(const cufftReal *input, 
                                                                       cufftComplex *output, 
                                                                 const int M, 
                                                                 const int N,
                                                                 const int halfM, 
                      ...
                      cufftReal*    output)
{
  ...
  idct_2d_fft(M, N, plan, expkMForInverse, expkNForInverse, expkMN1, expkMN2,
              workSpaceReal1, workSpaceComplex, workSpaceReal2, workSpaceReal3);
