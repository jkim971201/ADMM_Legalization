...
{
  setConfig(cfg);
  this->setZValue(seg->layer()->index() + 1);
  ...
  if(pathX1 == pathX2)
  {
    lx = static_cast<double>(pathX1 - width / 2.0);
    ux = static_cast<double>(pathX1 + width / 2.0);
    if(pathY1 < pathY2)
    {
      ly = static_cast<double>(pathY1 - width / 2.0);
      uy = static_cast<double>(pathY2 + width / 2.0);
    }
    else
    {
      ly = static_cast<double>(pathY2 - width / 2.0);
      uy = static_cast<double>(pathY1 + width / 2.0);
    }
  }
  else if(pathY1 == pathY2)
  {
    ...
    {
      lx = static_cast<double>(pathX1 - width / 2.0);
      ux = static_cast<double>(pathX2 + width / 2.0);
    }
    else
    {
      lx = static_cast<double>(pathX2 - width / 2.0);
      ux = static_cast<double>(pathX1 + width / 2.0);
    }
  }
  else
                      ...
                      QWidget* widget)
{
  ...
  getBrush().setStyle(Qt::BrushStyle::FDiagPattern);
  ...
  : net_(net) 
{
  ...
  { 
    ...
    double segLy = segRect.top();
    double segUx = segRect.right();
    double segUy = segRect.bottom();
